def english_number number
  ...
  ['quintillion', 18], ['sextillion', 21],['septillion', 24],['octillion', 27],['nonillion', 30],['decillion', 33],
...
class BeerSong
  ...
  def print_song
    while @beer > 0
    	puts "#{english_number(@beer).capitalize} #{@beer <= 1 ? "bottle" : "bottles"} of beer on the wall,"
    	puts "#{english_number(@beer).capitalize} #{@beer <= 1 ? "bottle" : "bottles"} of beer,"
    	puts "Take one down, pass it around,"
    	puts "#{english_number(@beer-1).capitalize} #{@beer-1 <= 1 ? "bottle" : "bottles"} of beer on the wall."
			@beer -= 1
	  end
  end
