...
def bottles num
  return "bottles" if num > 0 || num == 0
  return "bottle" if num == 1
end

until beers == 1
  puts "#{(english_number beers).capitalize} #{bottles beers} of beer on the wall, #{english_number beers} #{bottles beers} of beer!"
  puts "Take one down, pass it around, #{english_number beers - 1} #{bottles beers - 1} of beer on the wall!"
  beers -= 1
end
