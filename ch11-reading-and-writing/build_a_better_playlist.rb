...
# Commenting out my solution because the use of rand doesn't let it pass the specified rspec tests.
...
def music_shuffle filenames
  ...
  2.times do
  	...
  	counter = 0
  	bottom = 0
  	half = filenames.length/2

  	loop do
  	  if counter.even?
  	    new_arr << filenames[bottom]
  	    bottom += 1
  	  else
  	    new_arr << filenames[half]
  	    half += 1
  	  end
  	  break if new_arr.length == filenames.length
  	  counter += 1
  	end
  	filenames = new_arr
  ...
  loop do
    solution << filenames[cut]
    break if solution.length == filenames.length
    cut += 1
    cut = 0 if cut == filenames.length
  end
