  ...
  # it is not possible to pass the test on this one without knowing one of the algorithms that Chris Pine is
  # using as a potential solution.  The "better" shuffle it is, the more random and therefore imposible to
  # capture in a test, so the standard solution has been used.
  # my method of shuffle uses the internal time to create a "random" classification of files and therefore it
  # won't pass any tests
