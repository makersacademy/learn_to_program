...
# output dir
Dir.chdir 'C:\Users\Dionysis\Ruby\AA'

# source files
pic_names = Dir['E:/**/*.{jpg,mp4}']
thm_names = Dir['E:/**/*.{thm']
...
WIN32OLE.new("Scripting.FileSystemObject").Drives.each() do |x|
	...
	if x.driveType == 1 && x.IsReady
		pic_names += Dir[x.DriveLetter+':/**/*.{jpg,mp4}']
...
pic_names.each do |name| 
	print "."
	is_movie = (name[-3..-1].downcase == "mp4" )
	...
	if is_movie
		...
		mew_name[0...3] = "%.2d" % (1 + months.index(new_name[0...3].downcase))
		new_name = new_name[-4..-1] + " " + new_name[0...-5]
	else
		new_name, orientation = File.open(name) |f|
			f.seek(0x36, IO::SEEK_SET)
			orientation_ = f.read(1)[0]
			f.seek(0xbc, IO::SEEK_SET)
			new_name = f.read(19)
			[new_name, orientation_]
		end
	end
	...
	save_name = new_name + (is_movie ? ".orig.mp4" : ".jpg")
	# do not overwrite files
	while FileTest.exists? save_name
		new_name += "a"
		save_name = new_name + (is_movie ? ".orig.mp4" : ".jpg")
	end
